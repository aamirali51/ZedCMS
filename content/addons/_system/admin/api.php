...
function zed_get_admin_menu_items(): array
{
    ...
    // =========================================================================
    // ADDON-REGISTERED MENUS (via zed_register_admin_menu)
    // =========================================================================
    if (function_exists('zed_get_admin_menus')) {
        // Get list of active addons to filter menus
        $activeAddons = zed_get_option('active_addons', []);
        if (is_string($activeAddons)) {
            $activeAddons = json_decode($activeAddons, true) ?? [];
        }
        
        foreach (zed_get_admin_menus() as $id => $menu) {
            // Skip menus from disabled addons
            if (!empty($menu['registered_by']) && !in_array($menu['registered_by'], $activeAddons, true)) {
                continue;
            }
